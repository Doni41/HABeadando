...
<div class="container">
    ...
    <h1 class="mb-4">Címke szerkesztése</h1>

    @if (Session::has('label_updated'))
        <div class="alert alert-success">
            A címke szerkesztése sikeres volt!
        </div>
    @endif
    ...
    <form method="post" action="{{ route('labels.update', $label->id) }}">
        ...
        <div class="form-group row mb-3">
            ...
            <div class="col-sm-10 pt-2">
                <input type="text" class="form-control" id="color" name="color" value="{{ $label->color }}">
                @error('color')
                    {{ $message }}
                @enderror
            </div>
        ...
        <div class="form-group row mb-3">
            ...
            <div class="col-sm-10 pt-2">
                <input type="checkbox" class="form-check-input" id="flexCheckDefault" name="display" value="{{ $label->display  }}" {{ $label->display ? 'checked' : '' }}>
            </div>
        </div>
        <div class="text-center">
            <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i>Címke szerkesztése</button>
        </div>
